...
final class BasicController {
    ...
    func addRoutes(drop: Droplet) {
        ...
        basic.get("not-afks", handler:not_afks)
        basic.get("update", handler:update)
        basic.get("delete-afks", handler:delete_afks)
    }
    ...
    func not_afks(request: Request) throws -> ResponseRepresentable {
        return try JSON(node: Acronym.query().filter("short", .notEquals,"AFK").all().makeNode())
    }
    
    //GET 0.0.0.0:8080/basic/update?long=AwayFromFirst
    ...
    //GET 0.0.0.0:8080/delete-afks
    func delete_afks(request: Request) throws -> ResponseRepresentable {
